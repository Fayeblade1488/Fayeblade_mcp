...
import sys
import asyncio
from venice.bridge import run_main
...
async def main():
    ...
    try:
        io_timeout_s = int(os.environ.get("MCP_IO_TIMEOUT_S", "60"))
    except ValueError:
        io_timeout_s = 60
        print(f"[bridge] warning: invalid I/O timeout; falling back to {io_timeout_s}s", file=sys.stderr, flush=True)
